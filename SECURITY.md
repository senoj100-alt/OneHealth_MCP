...
OneHealth_MCP handles health and fitness data, so treat every deployment as sensitive.
...
OneHealth_MCP stores authenticated GitHub users and per-service connection records in Cloudflare D1. Fitness service credentials are encrypted with AES-GCM before being written to the `service_connections` table. Cloudflare KV is used for OAuth/session state, not as the primary multi-user credential database.
