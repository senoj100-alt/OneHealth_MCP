...
export interface OneHealthServiceStatus {
	id: OneHealthServiceId;
	label: string;
	configured: boolean;
...
export interface OneHealthServiceEnv {
...
const SERVICES: Array<Omit<OneHealthServiceStatus, "configured" | "source">> = [
	{
		id: "hevy",
		label: "Hevy",
		auth: "api_key",
...
function hasWorkerSecret(env: OneHealthServiceEnv, serviceId: OneHealthServiceId): boolean {
	switch (serviceId) {
		case "hevy":
...
export async function getOneHealthServiceStatuses(
	env: OneHealthServiceEnv,
	session: Pick<Props, "login" | "name" | "email">,
): Promise<OneHealthServiceStatus[]> {
