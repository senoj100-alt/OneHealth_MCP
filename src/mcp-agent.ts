...
export class MyMCP extends McpAgent<Env, Record<string, never>, Props> {
	...
	async init() {
		...
		if (!this.props || !this.props.login) {
			...
			throw new Error(
				"Authentication required. Please authenticate via OAuth to use OneHealth_MCP." +
					setupHint
			);
		}
		...
		this.server.tool(
			...
			async () => {
					try {
						...
						const statuses = await getOneHealthServiceStatuses(
							this.env,
							session,
						);
					...
					return {
						content: [
							{
								type: "text",
								text: `OneHealth service status for ${session.login}:\n${summary}`,
							},
							{
								type: "text",
				...
				content: [
					{
						type: "text",
						text: [
							"OneHealth MCP integration plan:",
